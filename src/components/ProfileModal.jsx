...
import { User, Calendar, MapPin, Users, ArrowRight } from "lucide-react";
...
const ProfileModal = ({ show, onClose, userMeetups, selectedMeetup, setSelectedMeetup, onLogout }) => {
  ...
  const pastMeetups = userMeetups.filter(m => {
    // Försök tolka datumsträngen (ex: "Lör, 2 Nov 2024 · 14:00")
      ...
      <div className="bg-background/90 rounded-3xl shadow-2xl max-w-xl w-full p-0 relative animate-fade-in overflow-hidden border border-border/60">
        ...
        <div className="relative h-40 bg-gradient-to-br from-primary/30 to-cyan-400/20 flex items-center justify-center">
          ...
          <button
            ...
            onClick={() => { onClose(); setSelectedMeetup(null); }}
            aria-label="Stäng"
          >
            ×
          </button>
        ...
        <div className="p-8">
          <h3 className="text-2xl font-bold mb-6 flex items-center gap-2 justify-center">Min Profil</h3>
          {!selectedMeetup ? (
            <>
              {/* Tabs */}
              <div className="flex gap-2 mb-6">
                <button
                  className={`px-4 py-2 rounded-lg font-semibold text-base transition-all ${activeTab === 'joined' ? 'bg-primary/90 text-white shadow' : 'bg-muted/40 text-muted-foreground hover:bg-muted/60'}`}
                  onClick={() => setActiveTab('joined')}
                >
                  Mina Meetups
                </button>
                <button
                  className={`px-4 py-2 rounded-lg font-semibold text-base transition-all ${activeTab === 'created' ? 'bg-primary/90 text-white shadow' : 'bg-muted/40 text-muted-foreground hover:bg-muted/60'}`}
                  onClick={() => setActiveTab('created')}
                >
                  Mina skapade Meetups
                </button>
                <button
                  className={`px-4 py-2 rounded-lg font-semibold text-base transition-all ${activeTab === 'past' ? 'bg-primary/90 text-white shadow' : 'bg-muted/40 text-muted-foreground hover:bg-muted/60'}`}
              ...
              {activeTab === 'joined' ? (
                userMeetups.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12">
                    ...
                    <p className="text-muted-foreground text-center">Du har inga anmälda meetups ännu.<br />Anmäl dig till ett event för att se det här!</p>
                  </div>
                ) : (
                    ...
                    {userMeetups.map(meetup => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-foreground group-hover:text-primary transition-colors">{meetup.title}</span>
                        </div>
                        <div className="flex items-center gap-3 text-sm text-muted-foreground pl-8">
                          <span><MapPin className="inline w-4 h-4 mr-1" />{meetup.location}</span>
                          <span><Users className="inline w-4 h-4 mr-1" />{meetup.attendees} deltagare</span>
                        </div>
                        <div className="pl-8 text-xs text-primary/80 mt-1">{meetup.date}</div>
                      </li>
                    ))}
                  </ul>
                )
                ...
                createdMeetups.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12">
                    <Calendar className="w-10 h-10 text-muted-foreground mb-2" />
                    <p className="text-muted-foreground text-center">Du har inte skapat några meetups ännu.</p>
                  </div>
                ) : (
                  <ul className="divide-y divide-border/60 rounded-xl overflow-hidden bg-muted/30 shadow-inner">
                ...
                pastMeetups.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12">
                    <Calendar className="w-10 h-10 text-muted-foreground mb-2" />
                    <p className="text-muted-foreground text-center">Du har inga tidigare meetups.</p>
                  </div>
                ) : (
                  <ul className="divide-y divide-border/60 rounded-xl overflow-hidden bg-muted/30 shadow-inner">
            ...
            <div className="animate-fade-in">
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="text-2xl font-bold text-foreground">{selectedMeetup.title}</h3>
              </div>
              <div className="flex flex-wrap gap-3 mb-2 text-muted-foreground text-sm">
                <span className="flex items-center gap-1"><MapPin className="w-4 h-4" />{selectedMeetup.location}</span>
                <span className="flex items-center gap-1"><Users className="w-4 h-4" />{selectedMeetup.attendees} deltagare</span>
                <span className="inline-block bg-primary/10 text-primary px-2 py-0.5 rounded text-xs font-semibold">{selectedMeetup.category}</span>
              </div>
              <div className="mb-2 text-xs text-muted-foreground">{selectedMeetup.date}</div>
              <div className="mt-6 bg-background/80 rounded-xl p-4 shadow-inner border border-border/40">
                <p className="text-base text-foreground">Mer information om detta meetup kan visas här.</p>
              </div>
            </div>
          )}
        </div>
